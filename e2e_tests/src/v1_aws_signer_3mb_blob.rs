...
async fn test_dispatch_3mb_blob_aws_signer() -> Result<()> {
    ...
    let secret_name = env::var("EIGEN_KEY").context(
        "Failed to read EIGEN_KEY (expected AWS Secret Name) environment variable",
    )?;
    ...
    let k256_signing_key = SigningKey::from(&secret_key);
