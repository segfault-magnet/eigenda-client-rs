...
#[derive(Debug)]
pub(crate) struct RawEigenClient<S> {
...
impl<S> RawEigenClient<S> {
    ...
    pub(crate) async fn new(
        ...
        let eth_client =
            eth_client::EthClient::new(url, config.eigenda_svc_manager_address);
    ...
    async fn dispatch_blob_non_authenticated(
            ...
            .map_err(BlobStatusError::Prost)?
        {
            ...
            | disperser::BlobStatus::Unknown => {
                Err(BlobStatusError::BlobDispatchedFailed)?
            }
    ...
    async fn dispatch_blob_authenticated(
        &self,
        data: Vec<u8>,
    ) -> Result<String, EigenClientError>
    where
        S: Signer,
    {
        ...
        let disperser::authenticated_reply::Payload::DisperseReply(disperse_reply) =
            reply
        else {
    ...
    pub(crate) async fn dispatch_blob(
        &self,
        data: Vec<u8>,
    ) -> Result<String, EigenClientError>
    where
    ...
    async fn receive_blob_auth_header(
        ...
        if let disperser::authenticated_reply::Payload::BlobAuthHeader(blob_auth_header) =
            reply
        {
            Ok(blob_auth_header)
        } else {
